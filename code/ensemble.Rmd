...
ensemble = function(p_model, h_model, h_model_var, df, ptype, htype, as){
  ...
  for(a in as){
    ...
    lglosses = c(LogLoss(ph_prob, y_val), lglosses  )
    aucs = c(AUC(ph_prob, y_val), aucs)
    accs = c(Accuracy(ph_pred, y_val), accs)
  }
  #return(en_f1)
...
hospital_model = function(train, val, imbalance = "NA"){
  ...
  pop_val = val
  population_model = readRDS(file = "rf_basetime_100.Rds")
  ptype ="rf"
  ...
  result = list("result_df" = summary_result, "best_hos_model" = best_model, "boruta" = boruta_features, "k_best" = k_best_features, "best_hos_model_en" = best_enmodel, "alpha" = best_alpha)
}
```
...
#Zining second half please use c(185:367)
half_n_of_hos = n_of_hos[c(1:184)]


for(h in half_n_of_hos){
  train = trains[[h]]
  val = vals[[h]]
  result = hospital_model(train, val, imbalance = "up")
...
ed = Sys.time()
print(ed - st)
saveRDS(hos_results, 'hos_results1_184.Rds')
...
write.csv(x = h_ns_best_hmodel, file = "h_ns_best_hmodel")
write.csv(x = h_ns_best_enmodel, file = "h_ns_best_enmodel")
```
