...
hospital_model = function(train, val, imbalance = "NA", population_model){
  ...
  result = list("result_df" = summary_result, "best_hos_model" = best_model, "boruta" = boruta_features, "k_best" = k_best_features, "best_hos_model_en" = best_enmodel, "alpha" = best_alpha, "best_hos_model_type" = best_hos_model_type, "best_en_model_type" = best_en_model_type)
  return(result)
}
```
...
```
```{r}
p_model   = readRDS(file = "rf_basetime_100.Rds")
```
...
for(h in n_of_hos[i:e]){
  ...
  filename =paste("hos_results_new/hos_results_new_", i, ".Rds", sep = "")
  saveRDS(rds, filename)
  i = i+1
}
ed = Sys.time()
print(ed - st)
#saveRDS(hos_results, 'hos_results121_140_new.Rds')
```
