...
keep = df_medicare %>% 
  group_by(prvdr_num) %>% 
  summarise(total = n(), re_pos = sum(readm_flag), re_neg = total - re_pos, er_pos = sum(er_90days), er_neg = total - er_pos, mor_pos = sum(mortality_90), mor_neg = total - mor_pos) %>% filter(total > 99 & re_pos > 10 & re_neg > 10 & er_pos > 10 & er_neg > 10 & mor_pos > 10 & mor_neg > 10 ) %>% pull(prvdr_num)
...
check_target = function(hos_num, df, set_type){
  target_rate = mean(df$target)
  if(target_rate == 0){
    print(paste( hos_num , set_type, "target all 0", sep=" "))
    return(hos_num)
  }else if(target_rate == 1){
    print(paste( hos_num , set_type, "target all 1", sep=" "))
    return(hos_num)
  }
  return("pass")
}
...
tgen = readRDS(file = "pop_tgen.Rds")
for(h in n_of_hos){
  ...
  h_check_tarin = check_target(h, h_train, "train")
  h_check_val = check_target(h, h_val, "val")
  h_check_test = check_target(h, h_test, "test")
  if(h_check_tarin != "pass"){
    check_train = c(check_train, h_check_tarin)
  }
  if(h_check_val != "pass"){
    check_val = c(check_val, h_check_val)
  }
  if(h_check_test != "pass"){
    check_test = c(check_test, h_check_test)
  }
  ...
  trains[[h]] = train[, !(colnames(train) %in% drops)]
  vals[[h]] = val[, !(colnames(val) %in% drops)]
  tests[[h]] = test[, !(colnames(test) %in% drops)]
  val_pop[[i]] = val
  test_pop[[i]] = test
  i = i+1
}
...
hospital_model = function(train, val, imbalance = "NA"){
  ...
  print(paste("Number of boruta features selected:", as.character(length(boruta_features)-1), sep = " "))
  ...
  top_10_2019 = c('prior_inp_only_los', 'total_prior_los', 'days_before_admsn', 'prior_hha_visits', 'clm_utlztn_day_cnt',  'index_los', 'clm_pps_cptl_drg_wt_num', 'prior_out_visits', 'prior_snf_los', 'target')
  train_set[[3]] = train[, top_10_2019]
  val_set[[3]] = val[, top_10_2019]
  ...
  k_best_features = c(k_best_results$featuresSelected, 'target')
  train_set[[4]] = train[, k_best_features]
  val_set[[4]] = val[, k_best_features]
  print(paste("Number of kbest features selected:", as.character(length(k_best_features)-1), sep = " "))
  print(Sys.time() - k_start_time)

  
  #training models
  model_types = c("rlg", "rf", "xgb")
  X_types = c("Boruta", "Boruta_time", "2019_top10", "selectKbest")
  model_X_types = c()
  cv_f1s = c()
  val_f1s = c()
  loglosses = c()
  ...
  result = list(summary_result, best_model, boruta_features, k_best_features)
}
```

```{r}
#for testing on 1 set
train = trains[[8]]
val = vals[[8]]
...
for(h in n_of_hos){
  ...
  result = hospital_model(train, val, imbalance = "up")
  hos_results[[i]] = result[[1]]
  best_models[[i]] =  result[[2]]
  burota[[i]] = result[[3]]
  kbest[[i]] = result[[4]]
  i = i+1
}
...
```
```{r}
saveRDS(best_models, '05_10_bestM.Rds')
saveRDS(hos_results, '05_10_result.Rds')
saveRDS(burota, '05_10_burota.Rds')
saveRDS(kbest, '05_10_kbest.Rds')
```
