...
test <- population_sample %>% group_by(prvdr_state_name) %>% summarise( length(prvdr_state_name) )
```

```{r}
# to obtain summary based on provider's state
df_medicare_state <- df_medicare %>% group_by(prvdr_state_name) %>% summarise(number = length(prvdr_state_name), sample = as.integer(length(prvdr_state_name)/20) )
```
```{r}
ggplot(data=df_medicare_state, aes(x = reorder(prvdr_state_name, -number), y=number)) +
...
```
```{r}
ggplot(data=test, aes(x = reorder(prvdr_state_name, -sample_size), y=sample_size)) +
  geom_bar(stat="identity", fill="steelblue") +
  coord_flip() +
  xlab('state')+ 
  ylab('sample size') +
...
```{r}
df_medicare_hos <- df_medicare %>% group_by(provider_type) %>% summarise(number = length(provider_type), sample = as.integer(length(provider_type)/20) )

ggplot(data=df_medicare_hos, aes(x = reorder(provider_type, -number), y=number)) +
  geom_bar(stat="identity", fill="steelblue") +
  coord_flip() +
  xlab('Hospital type')+ 
  ylab('number of patients') +
...
```{r}
test <- population_sample %>% group_by(provider_type) %>% summarise(sample_size = length(provider_type) )

ggplot(data=test, aes(x = reorder(provider_type, -sample_size), y=sample_size)) +
  geom_bar(stat="identity", fill="steelblue") +
  coord_flip() +
  xlab('hospital type')+ 
  ylab('Sample size') +
