...
#  df_medicare <- df_medicare[, !(colnames(df_medicare) %in% c(""))]
```
...
keep <- hos_sample %>% 
  group_by(prvdr_num) %>% 
  summarise(num = length(prvdr_num)) %>%
  filter(num > 99) %>% 
  pull(prvdr_num)
...
keep = hos_sample %>% 
  group_by(prvdr_num) %>% 
  summarise(sum_val = sum(hos_sample$readm_flag)) %>% 
  filter(sum_val >= 10) %>% 
  pull(prvdr_num)
hos_sample <- hos_sample %>% 
  filter(prvdr_num %in% keep)
...
keep = hos_sample %>% 
  group_by(prvdr_num) %>% 
  summarise(sum_val = sum(mortality_90)) %>% 
  filter(sum_val >= 10) %>% 
  pull(prvdr_num)
hos_sample <- hos_sample %>% 
  filter(prvdr_num %in% keep)
...
keep = hos_sample %>% 
  group_by(prvdr_num) %>% 
  summarise(sum_val = sum(readm_flag)) %>% 
  filter(sum_val >= 10) %>% 
  pull(prvdr_num)
hos_sample <- hos_sample %>% 
  filter(prvdr_num %in% keep)
...
sigtest <- Boruta(readm_flag~. , data = train_base, holdHistory=FALSE, doTrace=2)
boruta_features = getSelectedAttributes(sigtest)
train_bor = train[, boruta_features]
val_bor = val[, boruta_features]

## Boruta with time data set
boruta_time_features = c(boruta_features, time_feature)
train_bor_time = train[, boruta_time_features]
val_bor_time = val[, boruta_time_features]
