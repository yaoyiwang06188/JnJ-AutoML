...
# only need for xgboost
xg_process = function(train,val,test){
  train_xg = train %>% select(-readm_flag)
  train_xg = xgb.DMatrix(as.matrix(train_xg))
  val_xg = val %>% select(-readm_flag)
  val_xg = xgb.DMatrix(as.matrix(val_xg))
  test_xg = test %>% select(-readm_flag)
  test_xg = xgb.DMatrix(as.matrix(test_xg))
  
  return(list(train_xg,val_xg,test_xg))
}
...
data_preprocess = function(train,val,test,with_time,with_boruta,with_encoder){
 
  if (with_time == 0) {
    time_feature = c('month','week_of_dates','days_before_admsn')
    train = train[,!(colnames(train) %in% time_feature)]
    val = val[,!(colnames(val) %in% time_feature)]
    test = test[,!(colnames(test) %in% time_feature)]
  }
  
  if (with_boruta == 1){
    sigtest <- Boruta(readm_flag~. , data = train, holdHistory=FALSE, doTrace=2)
    boruta_features = getSelectedAttributes(sigtest)
    train = train[, boruta_features]
    val = val[, boruta_features]
  }
  
  if (with_encoder == 1){
    cat_index = match(names(Filter(is.factor, train)), names(train))
  ...
  # if (if_xg == 1){
  #   train_xg <- xg_process(train,val,test)[[1]]
  #   val_xg <- xg_process(train,val,test)[[2]]
  #   test_xg <- xg_process(train,val,test)[[3]]
  #   train <- train_xg
  #   val <- val_xg
  #   test <- test_xg
  # }
...
train_process <- data_preprocess(train,val,test,with_time=0,with_boruta=0,with_encoder=1)[[1]]
val_process <- data_preprocess(train,val,test,with_time=0,with_boruta=0,with_encoder=1)[[2]]

test_process <- data_preprocess(train,val,test,with_time=0,with_boruta=0,with_encoder=1)[[3]]
...
# comboInfo <- findLinearCombos(train_process)
# comboInfo
```
...
fitModel = function(train,type){
  
  folds <- 5
  cvIndex <- createFolds(train_process$readm_flag, folds, returnTrain = T)
...
model <- fitModel(train_process,type = 'rlg')
```
