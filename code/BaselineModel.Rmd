...
```{r}
#write the qualified hospital number into csv file 11447
...
summary_down50= population_model(train, val, imbalance = "down", sm_train_size, sm_val_size )
summary_down50_result = summary_down50[[1]]
down50_models =  summary_down50[[2]]
```
...
saveRDS(model_rf100, "rf_basetime_100.Rds")
d_t = Sys.time() - rf50_st
print(d_t)
```
...
write.csv(x = summary_result_rf100, file = "summary_result_rf100")
```
```{r}
model_rf100 = readRDS(file = "rf_basetime_100.Rds")
```
```{r}
d = read.csv("summary_result_rf100")
```
